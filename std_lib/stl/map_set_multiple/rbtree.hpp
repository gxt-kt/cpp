...
/*
 * Ref: https://github.com/parallel101/stl1weekend/blob/main/_RbTree.hpp
 *      https://www.bilibili.com/video/BV1oW42197Kc
 *
 *  关于红黑数，我之前也研究过，具体目录可以看本仓库cpp/algorithm/bst/RedBlackTree.hpp
 *
 */
...
struct _RbTreeBase {
 protected:
  ...
  explicit _RbTreeBase(_RbTreeRoot *_block) : _M_block(_block) {}
  ...
  static void _M_fix_violation(_RbTreeNode *__node) noexcept {
    while (true) {
      ...
      _RbTreeChildDir __parent_dir =
          __parent->_M_pparent == &__grandpa->_M_left ? _S_left : _S_right;
  ...
  template <class _NodeImpl, class _Tv, class _Compare>
  std::pair<_RbTreeNode *, _RbTreeNode *> _M_equal_range(
      _Tv &&__value, _Compare __comp) const noexcept {
  ...
  static void _M_delete_fixup(_RbTreeNode *__node) noexcept {
    ...
    while (__node->_M_parent != nullptr && __node->_M_color == _S_black) {
      _RbTreeChildDir __dir = __node->_M_pparent == &__node->_M_parent->_M_left
                                  ? _S_left
                                  : _S_right;
      _RbTreeNode *__sibling = __dir == _S_left ? __node->_M_parent->_M_right
...
struct _RbTreeNodeHandle<
  ...
  typename _Tp::first_type &key() const noexcept { return this->value().first; }
...
struct _RbTreeImpl : protected _RbTreeBase {
 ...
 public:
  ...
  std::pair<const_iterator, const_iterator> equal_range(
      _Tp const &__value) const noexcept {
    return {this->lower_bound(__value), this->upper_bound(__value)};
  }
 ...
 public:
  ...
  bool empty() const noexcept { return this->_M_block->_M_root == nullptr; }
